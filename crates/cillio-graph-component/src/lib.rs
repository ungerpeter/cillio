...
use crate::bindings::exports::cillio::graph::composition_graph::{
    Guest as CompositionGuest, GuestGraph as CompositionGuestGraph, Import, InstanceId, ItemKind,
};
use anyhow::Result;
use bindings::exports::cillio::graph::composition_graph::{
    Component, ComponentId, EncodeOptions, Export,
};
...
pub struct GraphComponent {
    composition_graph: RefCell<CompositionGraph<'static>>,
}

bindings::export!(GraphComponent with_types_in bindings);

impl CompositionGuest for GraphComponent {
    type Graph = Self;
}

impl CompositionGuestGraph for GraphComponent {
    fn new() -> Self {
        Self {
            composition_graph: RefCell::new(CompositionGraph::new()),
        }
    }
    ...
    fn add_component(&self, name: String, bytes: Vec<u8>) -> Result<Component, String> {
        ...
        let mut graph = self.composition_graph.borrow_mut();
    ...
    fn remove_component(&self, id: ComponentId) {
        self.composition_graph
            .borrow_mut()
            .remove_component(id as usize);
    }

    fn remove_instance(&self, id: InstanceId) {
        self.composition_graph
            .borrow_mut()
            .remove_instance(id as usize);
    }
    ...
    fn print_graph(&self) -> String {
        format!("{:#?}", self.composition_graph.borrow())
    }
